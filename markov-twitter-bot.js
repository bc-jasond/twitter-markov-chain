/**
 * this library's methods are all coupled together, I pulled it into the project and slightly modified it for this exercise but, wouldn't use it otherwise
 * */
...
class TwitterBot {
  ...
  getTweets(cb) {
    ...
    let get = (max_id) => {
      this.twitterClient.get('statuses/user_timeline', {screen_name: this.options.account, max_id: max_id, count: 200, exclude_replies: true, include_rts: false}, (error, timeline, response) => {
        ...
        if (count === 16) {
          if (cb) { return cb(null, this.arrayOfTweets) }
  ...
  generateTweet (callback) {
    ...
    if (callback) { return callback(null, tweet) }
